...
bool Graph<T>::removeVertex(const T &in) {
	for (auto it = vertexSet.begin(); it != vertexSet.end(); it++) {
	    if ((*it)->info == in) {
	        Vertex<T> *v = *it;
	        vertexSet.erase(it);
	        for (auto ite = vertexSet.begin(); ite != vertexSet.end(); ite++) {
                (*ite)->removeEdgeTo(v);
	        }
	        return true;
	    }
...
vector<T> Graph<T>::bfs(const T & source) const {
    ...
    while(!aux.empty()) {
        Vertex<T>* current = aux.front();
        aux.pop();
        res.push_back(current->info);
        for (auto edge : current->adj) {
            Vertex<T> * adjCurrent = edge.dest;
            if (adjCurrent->visited == false) {
                aux.push(adjCurrent);
                adjCurrent->visited = true;
            }
        }
    }
	return res;
...
vector<T> Graph<T>::topsort() const {
    ...
    queue<Vertex<T> *> aux;
    for (auto vert: vertexSet){
        if (vert->indegree == 0) aux.push(vert);
    }

    while(!aux.empty()) {
        Vertex<T> *adjCurrent = aux.front();
        aux.pop();
        res.push_back(adjCurrent->info);
        for (auto edge: adjCurrent->adj){
            edge.dest->indegree--;
